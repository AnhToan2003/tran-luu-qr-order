...
export class OrderService {
  static async placeOrder(sessionHash: string, raw: unknown): Promise<OrderDoc> {
    const input = placeOrderSchema.parse(raw);
    const fingerprint = createHash('sha256').update(JSON.stringify({courtCode:input.courtCode,items:[...input.items].sort((a,b)=>a.productId.localeCompare(b.productId))})).digest('hex');
    const c = getCollections();
    const existingQuery = {customerSessionHash:sessionHash,clientRequestId:input.clientRequestId};
    const check = (order: OrderDoc) => {
      if(order.requestFingerprint && order.requestFingerprint !== fingerprint) throw new ApiError(409,'REQUEST_CONFLICT','Mã yêu cầu đã dùng cho giỏ hàng khác');
      return order;
    };
    ...
    try {
      return await transaction(async session => {
        ...
        const config = await c.appSettings.findOneAndUpdate({key:'system_config','value.isAcceptingOrders':true},{$inc:{'value.orderRevision':1}},{session,returnDocument:'after'});
        if(!config) throw new ApiError(409,'SHOP_CLOSED','Quầy đang tạm dừng nhận đơn');
        const court = await c.courts.findOneAndUpdate({code:input.courtCode,isActive:true,deletedAt:null},{$set:{updatedAt:new Date()}},{session,returnDocument:'after'});
        ...
        for(const item of input.items) {
          ...
          orderItems.push({...item,nameSnapshot:product.name,volumeSnapshot:product.volume,unitPriceVnd:product.priceVnd,lineTotalVnd:product.priceVnd*item.quantity});
        ...
        const order: OrderDoc = {
          orderId,displayCode:`#TL-${day}-${String(counter!.value.sequence).padStart(4,'0')}`,
          ...existingQuery,courtId:court.courtId,courtNameSnapshot:court.name,requestFingerprint:fingerprint,
          items:orderItems,totalVnd:orderItems.reduce((sum,i)=>sum+i.lineTotalVnd,0),status:'accepted',
          createdAt:now,editableUntil:now,acceptedAt:now,updatedAt:now,version:1
        };
  ...
  static async transition(orderId: string, target: 'preparing'|'delivered') {
    return transaction(async session => {
      ...
      const now = new Date();
      const updated = await c.orders.findOneAndUpdate({orderId,status:order.status,version:order.version},{$set:{status:target,updatedAt:now,...(target==='delivered'?{deliveredAt:now}:{preparingAt:now})},$inc:{version:1}},{session,returnDocument:'after'});
      if(!updated) throw new ApiError(409,'CONFLICT','Đơn vừa được cập nhật, vui lòng tải lại');
      return updated;
    });
  }
  static async cancel(orderId: string, reason: string, admin: string) {
    return transaction(async session=>{
      ...
      const now = new Date();
      const updated = await c.orders.findOneAndUpdate({orderId,status:order.status,version:order.version},{$set:{status:'cancelled',cancelReason:reason,cancelledBy:admin,cancelledAt:now,updatedAt:now},$inc:{version:1}},{session,returnDocument:'after'});
      if(!updated) throw new ApiError(409,'CONFLICT','Đơn vừa được cập nhật');
